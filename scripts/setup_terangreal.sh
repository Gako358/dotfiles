...
nix --experimental-features "nix-command flakes" run github:nix-community/disko -- --mode zap_create_mount ./hosts/terangreal/disko.nix

# Check if disko completed successfully
if [ $? -eq 0 ]; then
    echo "Disk formatting completed successfully."
    echo "Installing NixOS..."
    nixos-install --flake .#terangreal --no-root-password
else
