...
#+begin_src emacs-lisp
  (use-package consult-gh
    ...
    ;; PRs -> open in pr-review (so we can actually review / approve /
    ;; merge). Falls back to consult-gh's built-in viewer if the URL
    ;; cannot be resolved or pr-review is unavailable.
