...
#+begin_src emacs-lisp
  ...
  (use-package corfu
    ...
    ;; Also remove ispell capf from buffer-local capfs in modes that re-add it
...
#+begin_src emacs-lisp
  ...
  (with-eval-after-load 'eglot
    ...
    (cl-defmethod eglot-initialization-options ((_server eglot-lsp-server)
                                                &context (major-mode scala-ts-mode))
      '(:defaultBspToBuildTool t
  			     :bloopInstall "off"
  			     :isFormatOnTypeEnabled :json-false)))
#+end_src
