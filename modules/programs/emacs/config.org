...
#+begin_src emacs-lisp
  (use-package copilot-chat
    ...
    ;; Auto-generate commit messages with copilot
...
#+begin_src emacs-lisp
  ...
  (use-package eca
    ...
    (eca-chat-custom-model "anthropic/claude-opus-4-7")
    ;; Show chat in a side window on the right
...
#+begin_src emacs-lisp
  ...
  (use-package apheleia
    ...
    ;; Add prettier for TypeScript, JavaScript, and Vue
    ...
    (push '(vue-ts-mode . prettier) apheleia-mode-alist)
...
#+begin_src emacs-lisp
  (use-package midnight
    :ensure t
    :config
    (setq clean-buffer-list-delay-general 36000)    ;; Kill regular buffers unused for 10h
    (setq clean-buffer-list-delay-special 36000)    ;; Kill special buffers unused for 10h
...
imports, ...). Debugging is provided by =dap-java= (bundled with
=dap-mode=, see [[*DAP][DAP]]); the test/main-class debug entry points
live alongside the rest of the Java leader bindings under =SPC l j=.
...
*** Postgres
PostgreSQL tooling. Two complementary entry points:
...
#+begin_src emacs-lisp
  ...
  (use-package sql
    ...
    :config
    ;; Persist comint history per product (psql, etc.)
  ...
  (use-package pgmacs
    ...
    (with-eval-after-load 'evil-leader
      (evil-leader/set-key
        ...
        "Dp" 'sql-postgres              ; Plain psql REPL
        "Dc" 'sql-connect)))            ; Use a named sql-connection-alist entry
#+end_src
...
enable it for Java (via =dap-java=, bundled with =dap-mode=, which
drives the JDT language server's debug plugin) and Scala (via
=dap-scala=, which talks to Metals' BSP-based DAP server — works for
both =sbt= and =bloop= projects).
...
#+begin_src emacs-lisp
  (use-package dap-mode
    ...
    (require 'dap-java)   ; Java   — uses lsp-java's JDT debug plugin
    (require 'dap-scala)  ; Scala  — uses Metals' DAP server over BSP
...
#+BEGIN_QUOTE
=diff-hl= overlaps with =git-gutter= below; both draw VC indicators in
the fringe. Pick one in practice — keep =git-gutter= disabled (or remove
it) if you switch to =diff-hl=, or vice versa. They are kept side by
side here so you can compare and choose.
#+END_QUOTE
