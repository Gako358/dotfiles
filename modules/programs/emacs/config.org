...
#+begin_src emacs-lisp
  ...
  (defun nixpkgs-fmt-before-save ()
    ...
    (when (eq major-mode 'nix-ts-mode)
  	...
  	(let ((output-buffer (get-buffer-create "*nixfmt*")))
  	  (shell-command-on-region (point-min) (point-max) "nixfmt" output-buffer t)
  	  (kill-buffer output-buffer))
        (message "nixfmt not found in PATH, skipping formatting."))))
