...
#+begin_src emacs-lisp
  ...
  project buffer is available."
    ...
    (evil-leader/set-key
      ...
      ;; NOTE: SPC TAB is reserved as the space-tree leader prefix
      ;; (see the space-tree use-package block).  Project-buffer
      ;; switch lives under SPC p b instead.
...
A lightweight, tree-based workspace manager built on top of
[[https://github.com/chiply/space-tree][space-tree]].  Each "space" is a
saved window configuration addressed by a list of integers
(e.g. =(2 1 3)= = third grandchild of the first child of the second
top-level space), so workspaces can be nested arbitrarily.

#+BEGIN_QUOTE
Instead of using =tab-bar-mode= or =eyebrowse=, we drive everything
through the standard completion stack — there is no tab bar, no extra
UI chrome.  Workspaces are listed and switched via =consult= and named
with =space-tree-name-current-space=; there is intentionally no
modeline indicator, since =SPC TAB TAB= already surfaces the full tree.
#+END_QUOTE

*Key ideas:*

+ *No tabs, no lighter* — =tab-bar= / =tab-bar-mode= stays disabled and
  the spaceline modeline is kept free of workspace chrome; navigate
  with =SPC TAB TAB= instead.
+ *Consult-driven* — =SPC TAB TAB= (and =gt=) opens a =consult= prompt
  listing every space in the tree, with depth-indented candidates,
  the current space marked, and named spaces shown by name.
+ *Tree-shaped* — siblings, children, and parents are all navigable;
  nest project-specific layouts under a top-level "Work" space and
  scratch ideas under a "Sandbox" space.
+ *Evil-native* — =gt= / =gT= override the standard vim "tabs" bindings
  to operate on spaces instead, and a full =SPC TAB= leader prefix
  covers create / delete / rename / copy / paste / navigate.
...
#+begin_src emacs-lisp
  (use-package space-tree
    ...
    :init
    (defvar my/consult-space-tree-history nil
      "History list for `my/consult-space-tree'.")

    ;; The helper commands and leader bindings live in `:init' (not
    ;; `:config'), so that:
    ;;   * `M-x my/consult-space-tree' is findable before space-tree
    ;;     has been autoloaded, and
    ;;   * the `SPC TAB ...' leader bindings are installed at startup
    ;;     (otherwise pressing SPC TAB TAB would never trigger an
    ;;     autoload of the package).
    ;; The functions explicitly `(require 'space-tree)' before they
    ;; touch package-internal variables.

    ;; --- Consult integration ---------------------------------------------
    ;; space-tree exposes its state in two hash tables; we walk them to
    ;; produce a flat, depth-ordered candidate list for `consult--read'.
    (defun my/space-tree--collect (node prefix last-flags)
      "Recursively collect all addresses under NODE with address PREFIX.
  LAST-FLAGS is a list of booleans (one per ancestor level) recording
  whether that ancestor was the last sibling at its level; it is used
  by the formatter to draw tree-branch glyphs.
  Returns a list of (ADDRESS NAME-OR-NIL LAST-FLAGS) entries in
  pre-order."
      (let* ((keys     (sort (hash-table-keys node) #'<))
             (last-key (car (last keys)))
             acc)
        (dolist (k keys)
          (let* ((addr     (append prefix (list k)))
                 (child    (gethash k node))
                 (name     (gethash addr space-tree-space-name-tbl))
                 (is-last  (equal k last-key))
                 (flags    (append last-flags (list is-last))))
            (push (list addr name flags) acc)
            (setq acc (append (nreverse
                               (my/space-tree--collect child addr flags))
                              acc))))
        (nreverse acc)))

    (defun my/space-tree--format-candidate (entry current)
      "Format ENTRY (ADDRESS NAME LAST-FLAGS) for the consult prompt.
  CURRENT is the address of the currently-selected space; the matching
  candidate is decorated with a small bullet.  Top-level spaces are
  rendered without a tree prefix and in bold so they stand out from
  their (indented) descendants, which are drawn with Unicode
  box-drawing branches."
      (pcase-let* ((`(,addr ,name ,last-flags) entry)
                   (depth    (length addr))
                   (top-p    (= depth 1))
                   ;; Build tree-branch prefix using box-drawing chars.
                   ;; Ancestor levels contribute "│  " (more siblings
                   ;; follow) or "   " (parent was the last sibling);
                   ;; the current level contributes "├─ " or "└─ ".
                   (prefix
                    (if top-p
                        ""
                      (concat
                       (mapconcat (lambda (last?)
                                    (if last? "   " "│  "))
                                  (butlast last-flags) "")
                       (if (car (last last-flags)) "└─ " "├─ "))))
                   (addr-str (mapconcat #'number-to-string addr "."))
                   (label    (or name (format "space %s" addr-str)))
                   (mark     (if (equal addr current)
                                 (propertize " ●" 'face 'success)
                               ""))
                   (addr-face (if top-p '(bold shadow) 'shadow))
                   (label-str (if top-p (propertize label 'face 'bold) label))
                   (display  (format "%s%s  %s%s"
                                     (propertize prefix 'face 'shadow)
                                     (propertize (format "[%s]" addr-str)
                                                 'face addr-face)
                                     label-str
                                     mark)))
        (propertize display 'space-tree-address addr)))

    (defun my/consult-space-tree ()
      "Switch to a space-tree workspace using `consult'.

  Walks the entire tree and lets you pick any space by number, address,
  or user-supplied name.  The current space is marked with =●= and
  candidates are indented by depth so the tree shape is visible."
      (interactive)
      ;; Force-load space-tree: this function is defined in `:init' so
      ;; it must trigger the autoload itself before reading any of the
      ;; package's internal variables.
      (require 'space-tree)
      (unless (bound-and-true-p space-tree-current-address)
        (space-tree-init))
      (unless space-tree-current-address
        (user-error "space-tree not initialised; run M-x space-tree-init"))
      (let* ((entries    (my/space-tree--collect space-tree-tree nil nil))
             (current    space-tree-current-address)
             (candidates (mapcar (lambda (e)
                                   (my/space-tree--format-candidate e current))
                                 entries))
             (default    (cl-find-if
                          (lambda (c)
                            (equal (get-text-property 0 'space-tree-address c)
                                   current))
                          candidates))
             (choice (consult--read
                      candidates
                      :prompt        "Space: "
                      :category      'space-tree-space
                      :history       'my/consult-space-tree-history
                      :require-match t
                      :sort          nil
                      :default       default
                      :lookup
                      (lambda (selected cands &rest _)
                        (when-let* ((s (car (member selected cands))))
                          (get-text-property 0 'space-tree-address s))))))
        (when choice
          (space-tree-switch-or-create choice))))

    (defun my/consult-space-tree-new (name)
      "Create a new sibling space and immediately name it NAME."
      (interactive "sName for new space: ")
      (space-tree-create-space-current-level)
      (unless (string-empty-p name)
        (space-tree-name-current-space name)))

    (defun my/consult-space-tree-new-top (name)
      "Create a new top-level space and immediately name it NAME."
      (interactive "sName for new top-level space: ")
      (space-tree-create-space-top-level)
      (unless (string-empty-p name)
        (space-tree-name-current-space name)))

    (defun my/consult-space-tree-new-sub (name)
      "Create a new sub-space (child of the current space) and name it NAME.
  The new child becomes the current space.  Uses `space-tree-switch-or-create'
  to pick the next free child number at depth +1 under the current address."
      (interactive "sName for new sub-space: ")
      (require 'space-tree)
      (unless space-tree-current-address
        (user-error "space-tree not initialised; run M-x space-tree-init"))
      (let* ((parent space-tree-current-address)
             (children (hash-table-keys (space-tree--get parent)))
             (next     (if children (1+ (apply #'max children)) 1)))
        (space-tree-switch-or-create (append parent (list next))))
      (unless (string-empty-p name)
        (space-tree-name-current-space name)))

    ;; --- Evil normal-state bindings --------------------------------------
    ;; Repurpose vim's tab bindings for spaces (we don't use tab-bar).
    (with-eval-after-load 'evil
      (evil-define-key 'normal 'global
        (kbd "gt") #'my/consult-space-tree
        (kbd "gT") #'space-tree-go-to-last-space
        (kbd "]w") #'space-tree-go-right
        (kbd "[w") #'space-tree-go-left))

    ;; --- Leader bindings under SPC TAB -----------------------------------
    (evil-leader/set-key
      "TAB TAB" 'my/consult-space-tree            ; switch via consult
      "TAB ."   'space-tree-switch-space-by-name  ; switch named spaces only
      "TAB ,"   'space-tree-go-to-last-space      ; toggle last visited
      "TAB n"   'my/consult-space-tree-new        ; new sibling (named)
      "TAB N"   'my/consult-space-tree-new-top    ; new top-level (named)
      "TAB s"   'my/consult-space-tree-new-sub    ; new sub-space / child (named)
      "TAB r"   'space-tree-name-current-space    ; rename current
      "TAB d"   'space-tree-delete-space          ; delete current
      "TAB l"   'space-tree-go-right              ; next sibling
      "TAB h"   'space-tree-go-left               ; prev sibling
      "TAB c"   'space-tree-copy-workspace        ; copy window layout
      "TAB p"   'space-tree-paste-workspace       ; paste as new sibling
      "TAB P"   (lambda () (interactive)          ; paste in place
                  (space-tree-paste-workspace t))
      "TAB R"   'space-tree-init                  ; reset entire tree
      ;; Quick numeric jumps to top-level spaces (1..9).
      "TAB 1"   'space-tree-to-1
      "TAB 2"   'space-tree-to-2
      "TAB 3"   'space-tree-to-3
      "TAB 4"   'space-tree-to-4
      "TAB 5"   'space-tree-to-5
      "TAB 6"   'space-tree-to-6
      "TAB 7"   'space-tree-to-7
      "TAB 8"   'space-tree-to-8
      "TAB 9"   'space-tree-to-9)

    :config
    ;; Make sure the tab-bar stays out of the way: space-tree is the
    ;; canonical workspace mechanism in this config.
    (when (fboundp 'tab-bar-mode) (tab-bar-mode -1))
    (setq tab-bar-show nil)

    ;; Initialise on first load (creates space 1).
    (unless (bound-and-true-p space-tree-current-address)
      (space-tree-init)))
#+end_src
