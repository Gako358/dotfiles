...
#+begin_src emacs-lisp
  (use-package proced
    ...
    ;; Show processes for all users (not just current user)
    (proced-filter 'all)

    ;; Sort by CPU usage by default (most intensive first)
    (proced-sort 'pcpu)

    ;; Use human-readable sizes (K, M, G)
    (proced-format-memory-flag t)

    ;; Custom formats with better column selection
    (proced-format-alist
     '((short user pid tree pcpu pmem start time (args comm))
       ...
       ;; Custom format focused on performance monitoring
       (performance pid tree pcpu pmem vsize rss state time (args comm))
       ;; Custom format for system administration
       (admin user pid ppid tree nice pcpu pmem state start time (args comm))))

    ;; Use medium format by default
    (proced-format 'medium)

    :bind
    ;; Global keybinding to quickly access proced
    ("C-x p p" . proced)

    :hook
    ;; Customize proced mode keybindings
    (proced-mode . (lambda ()
		     ;; Better navigation
		     (local-set-key (kbd "j") 'next-line)
		     (local-set-key (kbd "k") 'previous-line)
		     (local-set-key (kbd "l") 'proced-toggle-tree)
		     (local-set-key (kbd "a") 'proced-toggle-auto-update)

		     ;; Quick filters
		     (local-set-key (kbd "/ /") 'proced-filter-interactive)
		     (local-set-key (kbd "/ u") (lambda () (interactive) (proced-filter-interactive 'user)))
		     (local-set-key (kbd "/ c") (lambda () (interactive) (proced-filter-interactive 'all)))

		     ;; Quick sorting
		     (local-set-key (kbd "s c") (lambda () (interactive) (proced-sort-interactive 'pcpu)))
		     (local-set-key (kbd "s m") (lambda () (interactive) (proced-sort-interactive 'pmem)))
		     (local-set-key (kbd "s p") (lambda () (interactive) (proced-sort-interactive 'pid)))
		     (local-set-key (kbd "s t") (lambda () (interactive) (proced-sort-interactive 'time)))
		     (local-set-key (kbd "s s") (lambda () (interactive) (proced-sort-interactive 'start)))

		     ;; Format switching
		     (local-set-key (kbd "F") 'proced-format-interactive)

		     ;; Process actions
		     (local-set-key (kbd "K") 'proced-send-signal) ; Kill with signal selection
		     (local-set-key (kbd "r") 'proced-renice)      ; Change process priority

		     ;; Update controls
		     (local-set-key (kbd "g") 'proced-update)
		     (local-set-key (kbd "G") 'proced-revert)))
    ...
    ;; Add highlighting to update hook
    (add-hook 'proced-post-display-hook #'my/proced-highlight-high-usage)

    ;; Helper function to kill process by name
    ...
    ;; Function to show only processes consuming resources
