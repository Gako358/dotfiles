...
#+begin_src emacs-lisp
  ...
  (use-package vterm
    ...
    (defvar last-vterm-buffer nil "Holds the last visited vterm buffer.")
        ...
        (cond ((numberp n)
              ...
              ((null project-vterms)
               (let ((buf (vterm (generate-new-buffer-name (format "*vterm-%s-1*" project)))))
                 ...
                 (switch-to-buffer buf)
                 (setq last-vterm-buffer buf)))
              ((and last-vterm-buffer (eq (current-buffer) last-vterm-buffer))
               (switch-to-buffer (other-buffer)))
              (t
               (setq last-vterm-buffer (car (seq-intersection (buffer-list) project-vterms)))
               (switch-to-buffer last-vterm-buffer)))))

    (defun toggle-vterm-layout ()
