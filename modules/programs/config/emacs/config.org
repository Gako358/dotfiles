  ...
  (use-package vterm
    ...
    (defun toggle-vterm (&optional n)
      ...
      (let ((default-directory (or (vc-root-dir) default-directory)))
        (cond ((numberp n) (let ((buf (vterm (generate-new-buffer-name (format "*vterm-%d*" n))))) (push buf vterms) (switch-to-buffer buf)))
              ((null vterms) (let ((buf (vterm (generate-new-buffer-name "*vterm-1*")))) (push buf vterms) (switch-to-buffer buf)))
    ...
    (defun vterm-layout ()
      ...
      (if vterm-layout-active
        ...
        (let ((default-directory (or (vc-root-dir) default-directory)))
          ...
          (let ((left-vterm (vterm (generate-new-buffer-name "*vterm-1*"))))
            (push left-vterm vterms)
            (split-window-right)
            (other-window 1)
            (let ((top-right-vterm (vterm (generate-new-buffer-name "*vterm-2*"))))
              (push top-right-vterm vterms)
              (split-window-below)
              (other-window 1)
              (let ((bottom-right-vterm (vterm (generate-new-buffer-name "*vterm-3*"))))
                (push bottom-right-vterm vterms)
                (balance-windows)
                (select-window (get-buffer-window left-vterm))
                (setq vterm-layout-active t)))))))

    ;; Define a function to toggle a vterm buffer on the right
    (defun toggle-vterm-right ()
      ...
      (if (and vterm-right-buffer (buffer-live-p vterm-right-buffer))
        ...
        (let ((default-directory (or (vc-root-dir) default-directory)))
          (split-window-right)
          (other-window 1)
          (setq vterm-right-buffer (vterm (generate-new-buffer-name "*vterm-right*")))
          (push vterm-right-buffer vterms)
