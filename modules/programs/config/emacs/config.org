...
#+begin_src emacs-lisp
  (use-package copilot-chat
    ...
    (evil-leader/set-key
      ...
      "cm" 'copilot-chat-model
      "cM" 'copilot-chat-models
...
#+begin_src emacs-lisp
  ...
  ;; Buffers
  (defun my/projectile-switch-to-buffer-with-term ()
    "Switch to a buffer whose name includes 'term' using projectile."
    (interactive)
    (let ((buffers (projectile-project-buffer-names))
	  (filtered-buffers '()))
      (dolist (buffer buffers)
	(when (string-match-p "term" buffer)
	  (push buffer filtered-buffers)))
      (if filtered-buffers
	  (switch-to-buffer (completing-read "Switch to buffer: " filtered-buffers))
	(message "No buffers found with term: %s" "term"))))

  (evil-leader/set-key
    "br" 'rename-buffer
    "bd" 'evil-delete-buffer
    "TAB" 'projectile-switch-to-buffer
    "tb" 'my/projectile-switch-to-buffer-with-term)
#+end_src
...
#+begin_src emacs-lisp
  (use-package counsel
    ...
    (evil-leader/set-key
      ...
      "fb" 'counsel-grep
      "fg" 'counsel-git-grep
      "SPC" 'counsel-git-grep-at-cursor))
  ...
  (defun counsel-git-grep-at-cursor ()
    "Search using counsel-git-grep on the word at the cursor."
    (interactive)
  ...
  (use-package vterm
    ...
    (defun working-term ()
      ...
      (let* ((project (projectile-project-name))
  	   ...
  	   (buf-name (format "*working-term-%s*" project)))
  	(if project
...
#+begin_src emacs-lisp
  (use-package vterm-toggle
    ...
    (defun default-term ()
      ...
      (let* ((project (projectile-project-name))
	     (default-directory (projectile-project-root))
	     (buf-name (format "*default-term-%s*" project)))
	(if project
	    (if (get-buffer buf-name)
