...
#+begin_src emacs-lisp
  ...
  (defun my/projectile-switch-to-buffer-with-term ()
	...
	(when (string-match-p "term" buffer)
      ...
      (if filtered-buffers
	  (switch-to-buffer (completing-read "Switch to terminal: " filtered-buffers))
	(message "No buffers found with term: %s" "term"))))

  (evil-leader/set-key
    "br" 'rename-buffer
    "bd" 'kill-matching-buffers
    "SPC" 'my/projectile-switch-to-buffer-with-term)
#+end_src
...
#+begin_src emacs-lisp
  ...
  (defun my-toggle-eat-term ()
    "Toggle eat terminal at the root of the project."
    (interactive)
    (let* ((project-root (projectile-project-root))
           (eat-buffer-name (format "*eat: %s*" (projectile-project-name))))
      (if (get-buffer eat-buffer-name)
          (if (string= (buffer-name) eat-buffer-name)
              (previous-buffer)
            (switch-to-buffer eat-buffer-name))
        (let ((default-directory project-root))
          (eat)
          (rename-buffer eat-buffer-name)))))

  (evil-leader/set-key
    "tt" 'my-toggle-eat-term)
